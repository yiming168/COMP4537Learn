...
```bash
# Fetch latest main and prune stale remote refs
git fetch origin main --prune

# Reset current branch to match origin/main, discarding local changes
git reset --hard origin/main
```

- Notes:
  - `--prune` removes stale remote-tracking references.
  - `reset --hard` overwrites working tree and index; uncommitted changes are lost.
  - If unsure, stash first: `git stash push -m "WIP before reset"`.
...
- <Nov. 05, 2025> — added sql language learned from Amir class yesterday.
